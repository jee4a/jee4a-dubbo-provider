package com.jee4a.dubbo.provider.common.io.cache.redis;
...
public class RedisUtils {
	...
	public RedisUtils(String uri, JedisPoolConfig redisConfig,String masterName) {
		this.uri = uri;
		...
		for (String str : addressArr) {
			...
			log.error("sentinels 服务端 ip:" + ip + "  port:" + port);
		}
		 
		pool = new JedisSentinelPool(masterName,sentinels,redisConfig);
	...
	public List<String> blpop(String key) {
		...
		try {
			jedis = getResource();
			List<String> ret = jedis.blpop(key);
			return ret;
		} catch (Exception e) {
			returnBrokenResource(jedis);
			log.error(e.getMessage(), e);
	...
	public Boolean hexists(String key, String fields) {
		...
		try {
			jedis = getResource();
			return jedis.hexists(key, fields);
		} catch (Exception e) {
			returnBrokenResource(jedis);
			log.error(e.getMessage(), e);
			throw new JedisException(e);
	...
	public boolean electioneer(String key, String candidates, int timeOut) {
		...
		try {
			jedis = getResource();
			int setnx = jedis.setnx(key, candidates).intValue();
			boolean bool = false;
			if (setnx == 1) {
				jedis.expire(key, timeOut);
				bool = true;
			} else {
				String electee = jedis.get(key);
	...
	public boolean canBeSubmit(String key, String value, int seconds) {
		...
		try {
			jedis = getResource();
			return "OK".equalsIgnoreCase(jedis.set(key, value, "NX", "EX", seconds));
		} catch (Exception e) {
			returnBrokenResource(jedis);
			log.error(e.getMessage(), e);
		} finally {
	 ...
	 * 的中心点是由给定的位置元素决定的， 而不是像 GEORADIUS 那样， 使用输入的经度和纬度来决定中心点 指定成员的位置被用作查询的中心。
	 * 
	 * 
